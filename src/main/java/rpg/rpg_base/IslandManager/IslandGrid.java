...
import rpg.rpg_base.IslandManager.events.IslandChangeOwnerEvent;
import rpg.rpg_base.IslandManager.events.IslandPreTeleportEvent;
import rpg.rpg_base.RPG_Base;
import rpg.rpg_base.data.Util.Util;
import rpg.rpg_base.data.Util.teleport.SafeTeleportBuilder;
...
public class IslandGrid {
    ...
    /**
     * @param plugin - RPG_Base plugin object
     */
    public IslandGrid(RPG_Base plugin) {
        this.plugin = plugin;
        loadGrid();
    }
            ...
            try {
                ...
                if (islandYaml.contains(Settings.worldName)) {
                    ...
                    if (islandYaml.contains("spawn")) {
                        ...
                        // Validate entries
                        if (spawnLoc != null && spawnLoc.getWorld() != null && spawnLoc.getWorld().equals(RPG_Base.getIslandWorld())) {
    ...
    protected boolean inWorld(Location loc) {
        if (loc.getWorld().equals(RPG_Base.getIslandWorld())) {
            return true;
        }
        return false;
    }
    ...
    public Island getIslandAt(int x, int z) {
        Map.Entry<Integer, TreeMap<Integer, Island>> en = islandGrid.floorEntry(x);
        if (en != null) {
            Map.Entry<Integer, Island> ent = en.getValue().floorEntry(z);
            if (ent != null) {
...
//        }
    ...
    public static boolean isSafeLocation(final Location l) {
        ...
        if (ground.getType().equals(Material.CACTUS) || ground.getType().name().contains("FENCE") || ground.getType().name().contains("SIGN")) {
        ...
        if (space1.getType().isSolid() && !space1.getType().name().contains("SIGN")){
            return false;
        }
        return !space2.getType().isSolid() || !space2.getType().name().contains("SIGN");
    }
    ...
    public Location getSafeHomeLocation(final UUID p, int number) {
        ...
        if (l != null) {
            ...
            if (isSafeLocation(l)) {
                return l.clone().add(0.5D,0D,0.5D);
            }
            ...
            if (lPlusOne != null) {
                if (isSafeLocation(lPlusOne)) {
                    ...
                    return lPlusOne.clone().add(0.5D,0,0.5D);
                }
            }
        }
        ...
        if (plugin.getPlayers().inTeam(p)) {
            ...
            if (isSafeLocation(l)) {
                ...
                return l.clone().add(0.5D,0,0.5D);
            } else {
                ...
                if (tlh != null) {
                    if (isSafeLocation(tlh)) {
                        ...
                        return tlh.clone().add(0.5D,0,0.5D);
                    }
                }
            }
        } else {
        ...
        for (Player player : plugin.getServer().getOnlinePlayers()) {
            if (island.inIslandSpace(player.getLocation())) {
                ...
                if (!player.getUniqueId().equals(uuid) && (plugin.getPlayers().hasIsland(player.getUniqueId()) || plugin.getPlayers().inTeam(player.getUniqueId()))) {
                    ...
                    if (spawn != null) {
                        // go to island spawn
                        player.teleport(RPG_Base.getIslandWorld().getSpawnLocation());
    ...
    public HashMap<String, Island> getUnownedIslands() {
        ...
        for (Map.Entry<Integer, TreeMap<Integer, Island>> x : islandGrid.entrySet()) {
    ...
    public void setSpawnPoint(Location location) {
        RPG_Base.getIslandWorld().setSpawnLocation(location.getBlockX(), location.getBlockY(), location.getBlockZ());
