...
import rpg.rpg_base.GeneralEvents.PlayerEvents;
import rpg.rpg_base.IslandManager.*;
import rpg.rpg_base.IslandManager.events.*;
...
public class IslandCommands implements CommandExecutor {
     ...
     * Constructor
     */
    public IslandCommands(RPG_Base RPG_Base) {
        // Plugin instance
    ...
    /**
     * Loads NBTs from the config.yml file. If the default
     * island is not included, it will be made up
     */
    public void loadNBTs() {
        // Check if there is a NBT folder and make it if it does not exist
        File NBTFolder = new File(plugin.getDataFolder(), "NBTs");
        if (!NBTFolder.exists()) {
            NBTFolder.mkdir();
        }
        // Clear the NBT list that is kept in memory
        NBTs.clear();
        // Load the default NBT if it exists
        // Set up the default NBT
        File NBTFile = new File(NBTFolder, "island.NBT");
        if (!NBTFile.exists()) {
            //plugin.getLogger().info("Default NBT does not exist...");
            // Only copy if the default exists
            if (plugin.getResource("NBTs/island.NBT") != null) {
                plugin.getLogger().info("Default NBT does not exist, saving it...");
                plugin.saveResource("NBTs/island.NBT", false);
                // Add it to NBTs
                try {
                    NBTs.put("default", new NBT(plugin, NBTFile));
                } catch (IOException e) {
                    plugin.getLogger().severe("Could not load default NBT!");
                    e.printStackTrace();
                }
                // If this is repeated later due to the NBT config, fine, it will only add info
            } else {
                // No islands.NBT in the jar, so just make the default using
                // built-in island generation
                NBTs.put("default", new NBT(plugin));
            }
            plugin.getLogger().info("Loaded default nether NBT");
        } else {
            // It exists, so load it
            try {
                NBTs.put("default", new NBT(plugin, NBTFile));
                plugin.getLogger().info("Loaded default island NBT.");
            } catch (IOException e) {
                plugin.getLogger().severe("Could not load default NBT!");
                e.printStackTrace();
            }
        }
        // Set up some basic settings just in case the NBTs section is missing
        if (NBTs.containsKey("default")) {
            NBTs.get("default").setIcon(Material.GRASS);
            NBTs.get("default").setOrder(1);
            NBTs.get("default").setName("The Original");
            NBTs.get("default").setDescription("");
            NBTs.get("default").setPartnerName("nether");
            NBTs.get("default").setBiome(Settings.defaultBiome);
            NBTs.get("default").setIcon(Material.GRASS);
            if (Settings.chestItems.length == 0) {
                NBTs.get("default").setUseDefaultChest(false);
            } else {
                NBTs.get("default").setUseDefaultChest(true);
            }
        }

        // Load the NBTs from config.yml
        ConfigurationSection schemSection = plugin.getConfig().getConfigurationSection("NBTsection");
        if (plugin.getConfig().contains("NBTsection")) {
            Settings.chooseIslandRandomly = schemSection.getBoolean("chooseislandrandomly", false);
            ConfigurationSection NBTsSection = schemSection.getConfigurationSection("NBTs");
            // Section exists, so go through the various sections
            for (String key : NBTsSection.getKeys(false)) {
                try {
                    NBT newSchem = null;
                    // Check the file exists
                    //plugin.getLogger().info("DEBUG: NBTs." + key + ".filename" );
                    String filename = schemSection.getString("NBTs." + key + ".filename", "");
                    if (!filename.isEmpty()) {
                        //plugin.getLogger().info("DEBUG: filename = " + filename);
                        // Check if this file exists or if it is in the jar
                        NBTFile = new File(NBTFolder, filename);
                        // See if the file exists
                        if (NBTFile.exists()) {
                            newSchem = new NBT(plugin, NBTFile);
                        } else if (plugin.getResource("NBTs/" + filename) != null) {
                            plugin.saveResource("NBTs/" + filename, false);
                            newSchem = new NBT(plugin, NBTFile);
                        }
                    } else {
                        ...
                        if (key.equalsIgnoreCase("default")) {
                            ...
                            newSchem = NBTs.get("default");
                        } else {
                            plugin.getLogger().severe("NBT " + key + " does not have a filename. Skipping!");
                        }
                    }
                    if (newSchem != null) {
                        // Set the heading
                        ...
                        // Order
                        newSchem.setOrder(schemSection.getInt("NBTs." + key + ".order", 0));
                        // Load the rest of the settings
                        // Icon
                        try {

                            Material icon;
                            String iconString = schemSection.getString("NBTs." + key + ".icon", "MAP").toUpperCase();
                            // Support damage values
                            String[] split = iconString.split(":");
                            if (StringUtils.isNumeric(split[0])) {
                                icon = Material.getMaterial(String.valueOf(Integer.parseInt(split[0])));
                                if (icon == null) {
                                    icon = Material.MAP;
                                    plugin.getLogger().severe("NBT's icon could not be found. Try using quotes like '17:2'");
                                }
                            } else {
                        ...
                        // Friendly name
                        String name = ChatColor.translateAlternateColorCodes('&', schemSection.getString("NBTs." + key + ".name", ""));
                        newSchem.setName(name);
                        // Rating - Rating is not used right now
                        int rating = schemSection.getInt("NBTs." + key + ".rating", 50);
                        if (rating < 1) {
                            rating = 1;
                        } else if (rating > 100) {
                            rating = 100;
                        ...
                        // Cost
                        double cost = schemSection.getDouble("NBTs." + key + ".cost", 0D);
                        if (cost < 0) {
                            cost = 0;
                        }
                        // Description
                        String description = ChatColor.translateAlternateColorCodes('&', schemSection.getString("NBTs." + key + ".description", ""));
                        ...
                        // Permission
                        String perm = schemSection.getString("NBTs." + key + ".permission", "");
                        newSchem.setPerm(perm);
                        // Use default chest
                        newSchem.setUseDefaultChest(schemSection.getBoolean("NBTs." + key + ".useDefaultChest", true));
                        // Biomes - overrides default if it exists
                        String biomeString = schemSection.getString("NBTs." + key + ".biome", Settings.defaultBiome.toString());
                        Biome biome = null;
                        try {
                        ...
                        newSchem.setUsePhysics(schemSection.getBoolean("NBTs." + key + ".usephysics", Settings.usePhysics));
                        // Paste Entities or not
                        newSchem.setPasteEntities(schemSection.getBoolean("NBTs." + key + ".pasteentities", false));
                        // Paste air or not.
                        newSchem.setPasteAir(schemSection.getBoolean("NBTs." + key + ".pasteair", true));
                        // Visible in GUI or not
                        newSchem.setVisible(schemSection.getBoolean("NBTs." + key + ".show", true));
                        // Partner NBT
                        if (biome != null && biome.equals(Biome.NETHER_WASTES)) {
                            // Default for nether biomes is the default overworld island
                            newSchem.setPartnerName(schemSection.getString("NBTs." + key + ".partnerNBT", "default"));
                        } else {
                            // Default for overworld biomes is nether island
                            newSchem.setPartnerName(schemSection.getString("NBTs." + key + ".partnerNBT", "nether"));
                        }
                        // Island companion
                        List<String> companion = schemSection.getStringList("NBTs." + key + ".companion");
                            ...
                            for (String companionType : companion) {
                                ...
                                if (companionType.equalsIgnoreCase("NOTHING")) {
                                    ...
                                    try {
                                        ...
                                        plugin.getLogger()
                                                .warning(
                                                        "Island companion is not recognized in NBT '" + name + "'.");
                                    }
                                }
                            }
                        ...
                        // Companion names
                        List<String> companionNames = schemSection.getStringList("NBTs." + key + ".companionnames");
                        ...
                        // Get chest items
                        final List<String> chestItems = schemSection.getStringList("NBTs." + key + ".chestItems");
                                ...
                                try {
                                    ...
                                    if (amountdata[0].equals("POTION")) {
                                        ...
                                        if (StringUtils.isNumeric(amountdata[0])) {
                                            mat = Material.getMaterial(String.valueOf(Integer.parseInt(amountdata[0])));
                                        } else {
                                    ...
                                    plugin.getLogger().severe("Problem loading chest item for NBT '" + name + "' so skipping it: " + chestItemString);
                        ...
                        // Player spawn block
                        String spawnBlock = schemSection.getString("NBTs." + key + ".spawnblock");
                        if (spawnBlock != null) {
                            // Check to see if this block is a valid material
                            try {
                                Material playerSpawnBlock;
                                if (StringUtils.isNumeric(spawnBlock)) {
                                    playerSpawnBlock = Material.getMaterial(String.valueOf(Integer.parseInt(spawnBlock)));
                                } else {
                                ...
                                if (newSchem.setPlayerSpawnBlock(playerSpawnBlock)) {
                                    ...
                                    plugin.getLogger().severe("Problem with NBT '" + name + "'. Spawn block '" + spawnBlock + "' not found in NBT or there is more than one. Skipping...");
                                }
                            } catch (Exception e) {
                                plugin.getLogger().severe("Problem with NBT '" + name + "'. Spawn block '" + spawnBlock + "' is unknown. Skipping...");
                            }
                        } else {
                            // plugin.getLogger().info("No spawn block found");
                        }
                        // Level handicap
                        newSchem.setLevelHandicap(schemSection.getInt("NBTs." + key + ".levelHandicap", 0));

                        // Store it
                        NBTs.put(key, newSchem);
                        if (perm.isEmpty()) {
                            perm = "all players";
                        } else {
                            perm = "player with " + perm + " permission";
                        }
                        plugin.getLogger().info("Loading NBT " + ChatColor.stripColor(name) + " (" + filename + ") for " + perm + ", order " + newSchem.getOrder());
                    } else {
                        plugin.getLogger().warning("Could not find " + filename + " in the NBTs folder! Skipping...");
                    }
                } catch (IOException e) {
                    plugin.getLogger().info("Error loading NBT in section " + key + ". Skipping...");
                }
            }
            if (NBTs.isEmpty()) {
                tip();
    ...
    private void tip() {
        // There is no section in config.yml. Save the default NBT anyway
        plugin.getLogger().warning("***************************************************************");
        plugin.getLogger().warning("* 'NBTs' section in config.yml has been deprecated.     *");
        plugin.getLogger().warning("* See 'NBTsection' in config.new.yml for replacement.   *");
        plugin.getLogger().warning("***************************************************************");
    }
    ...
    /**
     * List NBTs this player can access. If @param ignoreNoPermission is true, then only
     * NBTs with a specific permission set will be checked. I.e., no common NBTs will
     * be returned (including the default one).
     *
     * @param player
     * @param ignoreNoPermission
     * @return List of NBTs this player can use based on their permission level
     */
    public List<NBT> getNBTs(Player player, boolean ignoreNoPermission) {
        List<NBT> result = new ArrayList<>();
        // Find out what NBTs this player can choose from
        //Bukkit.getLogger().info("DEBUG: Checking NBTs for " + player.getName());
        for (NBT NBT : NBTs.values()) {
            //Bukkit.getLogger().info("DEBUG: NBT name is '"+ NBT.getName() + "'");
            //Bukkit.getLogger().info("DEBUG: perm is " + NBT.getPerm());
            if ((!ignoreNoPermission && NBT.getPerm().isEmpty()) || player.hasPermission(NBT.getPerm())) {
                //Bukkit.getLogger().info("DEBUG: player can use this NBT");
        ...
        Collections.sort(result, new Comparator<NBT>() {

            @Override
            public int compare(NBT o1, NBT o2) {
     ...
     * @return the NBTs
     */
    public static HashMap<String, NBT> getNBTs() {
        return NBTs;
    }
    ...
    public void newIsland(final Player player) {
        ...
        newIsland(player, NBTs.get("default"));
    }

    /**
     * Makes an island using NBT. No permission checks are made. They have to be decided
     * before this method is called.
     *
     * @param player
     * @param NBT
     */
    public void newIsland(final Player player, final NBT NBT) {
        ...
        // Set the biome
        //BiomesPanel.setIslandBiome(next, NBT.getBiome());
        // Teleport to the new home
        if (NBT.isPlayerSpawn()) {
            // Set home and teleport
            plugin.getPlayers().setHomeLocation(playerUUID, NBT.getPlayerSpawn(next), 1);
        ...
        // Create island based on NBT
        if (NBT != null) {
            //plugin.getLogger().info("DEBUG: pasting NBT " + NBT.getName() + " " + NBT.getPerm());
            //plugin.getLogger().info("DEBUG: nether world is " + RPG_Base.getNetherWorld());
            // Over world start
            //plugin.getLogger().info("DEBUG: pasting");
            //long timer = System.nanoTime();
            // Paste the island and teleport the player home
            NBT.pasteNBT(next, player, true, firstTime ? rpg.rpg_base.data.NBT.PasteReason.NEW_ISLAND : rpg.rpg_base.data.NBT.PasteReason.RESET);
            // Record the rating of this NBT - not used for anything right now
            plugin.getPlayers().setStartIslandRating(playerUUID, NBT.getRating());
        }
        ...
        // Done - fire event
        final IslandNewEvent event = new IslandNewEvent(player, NBT, myIsland);
        plugin.getServer().getPluginManager().callEvent(event);
     ...
     * @param player
     */
    private void pastePartner(final NBT NBT, final Location loc, final Player player) {
        plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable() {

            @Override
            public void run() {
                NBT.pasteNBT(loc, player, false, rpg.rpg_base.data.NBT.PasteReason.PARTNER);
                if (NBT.isPlayerSpawn()) {
                    // Set partner home
                    plugin.getPlayers().setHomeLocation(player.getUniqueId(), NBT.getPlayerSpawn(loc), -2);
                }
            }
        }, 60L);
    ...
    /**
     * Pastes a NBT at a location for the player
     *
     * @param NBT
     * @param loc
     * @param player
     * @param reason
     */
    public void pasteNBT(final NBT NBT, final Location loc, final Player player, rpg.rpg_base.data.NBT.PasteReason reason) {
        NBT.pasteNBT(loc, player, false, reason);
    }
    ...
    private Location getNextIsland(UUID playerUUID) {
        ...
        if (last == null) {
            last = new Location(RPG_Base.getIslandWorld(), Settings.islandXOffset + Settings.islandStartX, Settings.islandHeight, Settings.islandZOffset + Settings.islandStartZ);
        }
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] split) {
        if (!(sender instanceof Player)) {
            ...
            Util.sendMessage(sender.getServer().getPlayer(sender.getName()), plugin.myLocale(player.getUniqueId()).errorUseInGame);
            return false;
        }
        final Player player = (Player) sender;
        // Basic permissions check to even use /island
        if (!player.hasPermission( "island.create")) {
        ...
        if (split.length > 1 && split[0].equalsIgnoreCase("name")) {
            // Naming of island
            if (player.hasPermission( "island.name")
                ...
                Util.sendMessage(player, ChatColor.RED + "You don't have required permission");
                return true;
            }
        }
        ...
        switch (split.length) {
            ...
            case 0:
                ...
                if (plugin.getPlayers().getIslandLocation(playerUUID) == null && !plugin.getPlayers().inTeam(playerUUID)) {
                    ...
                    if (player.hasPermission( "island.controlpanel") && plugin.getPlayers().getControlPanel(playerUUID)) {
                        Util.runCommand(player, Settings.ISLANDCOMMAND + " cp");
                    } else {
                        // Check permission
                        if (!player.hasPermission( "island.go")) {
                            Util.sendMessage(player, ChatColor.RED + "You don't have required permission");
                            return true;
                        }
            ...
            case 1:
                if (split[0].equalsIgnoreCase("value")) {
                    ...
                    if (player.hasPermission( "island.name")
                        ...
                        Util.sendMessage((Player) sender, plugin.myLocale(player.getUniqueId()).generalSuccess);
                    }
                    return true;
                }
                if (split[0].equalsIgnoreCase("coop")) {
                    // Explain command
                    if (player.hasPermission( "coop")) {
                    ...
                    // Explain command
                    if (player.hasPermission( "island.expel")) {
                    ...
                    if (player.hasPermission( "team.chat")) {
                        ...
                        if (!Settings.teamChat) {
                            Util.sendMessage(player, ChatColor.RED + plugin.myLocale(player.getUniqueId()).errorUnknownCommand);
                            return false;
                        }
                        // Check if in team
                ...
                if (split[0].equalsIgnoreCase("banlist")) {
                    if (player.hasPermission( "island.ban")) {
                        // Show banned players
                    ...
                    return true;
                } else if (split[0].equalsIgnoreCase("unban") && player.hasPermission( "island.ban")) {
                    if (player.hasPermission( "island.ban")) {
                    ...
                    if (!player.hasPermission( "island.lock")) {
                        Util.sendMessage(player, ChatColor.RED + "You don't have required permission");
                        return true;
                    }
                    ...
                    if (island == null) {
                        ...
                        if (!island.isLocked()) {
                            ...
                            for (Player target : plugin.getServer().getOnlinePlayers()) {
                                if (target == null || target.hasMetadata("NPC") || target.isOp() || player.equals(target) || target.hasPermission( Settings.PERMPREFIX + "mod.bypassprotect"))
                                    continue;
                ...
                if (split[0].equalsIgnoreCase("warp")) {
                    if (player.hasPermission( "island.warp")) {
                    ...
                    if (!player.hasPermission( "island.reset")) {
                        Util.sendMessage(player, ChatColor.RED + "You don't have required permission");
                        return true;
                    }
                    ...
                    if (confirm.containsKey(playerUUID) && confirm.get(playerUUID)) {
                        ...
                        List<NBT> schems = getSchematics(player, false);
                    ...
                    if (player.hasPermission( "island.sethome")) {
                        // Check island
                        if (plugin.getGrid().getIsland(player.getUniqueId()) == null) {
                            Util.sendMessage(player, ChatColor.RED + plugin.myLocale(player.getUniqueId()).errorNoIsland);
                            return true;
                        }
                        plugin.getGrid().homeSet(player, 1);
                        return true;
                    } else {
                        Util.sendMessage(player, ChatColor.RED + plugin.myLocale(playerUUID).errorNoPermission);
                        return true;
                    ...
                    if (maxHomes > 1 && player.hasPermission( "island.go")) {
                    ...
                    } else if (player.hasPermission( "island.go")) {
                    ...
                    if (plugin.getGrid() != null && plugin.getGrid().getSpawn() != null && player.hasPermission( "island.spawn")) {
                    ...
                    if (player.hasPermission( "island.controlpanel")) {
                    ...
                    if (player.hasPermission( "island.info")) {
                    ...
                    if (player.hasPermission( "island.topten")) {
                    ...
                    if (Settings.useEconomy && player.hasPermission( "island.minishop")) {
                    ...
                    if (player.hasPermission( "island.value")) {
                    ...
                    if (player.hasPermission( "team.kick")) {
                    ...
                    if (player.hasPermission( "team.makeleader")) {
                    ...
                    if (player.hasPermission( "island.biomes")) {
                    ...
                    if (player.hasPermission( "island.settings")) {
                    ...
                    if (player.hasPermission( "island.challenges")) {
                    ...
                    if (player.hasPermission( "island.lang")) {
                 ...
                 */
                    ...
                    if (player.hasPermission( "island.spawn")) {
                        // go to spawn
                        Location l = RPG_Base.getIslandWorld().getSpawnLocation();
            ...
            case 2:
                if (split[0].equalsIgnoreCase("controlpanel") || split[0].equalsIgnoreCase("cp")) {
                    if (player.hasPermission( "island.controlpanelonoff")) {
                    ...
                    if (!NBTs.containsKey(split[1])) {
                        Util.sendMessage(player, ChatColor.RED + "Unknown command");
                        return true;
                    } else {
                        NBT schematic = NBTs.get(split[1]);
                        // Check perm again
                    ...
                    if (split[0].equalsIgnoreCase("go")) {
                        ...
                        if (player.hasPermission("team.kick")) {
                            if (!plugin.getPlayers().inTeam(playerUUID)) {
                                Util.sendMessage(player, ChatColor.RED + "Kick error no team");
                                return true;
                            }
                            // Only leaders can kick
                            if (teamLeader != null && !teamLeader.equals(playerUUID)) {
                                Util.sendMessage(player, ChatColor.RED + "Only leader can kick from team!");
                                return true;
                            }
                            ...
                            if (teamMembers.contains(targetPlayer)) {
                                ...
                                if (target != null) {
                                    ...
                                    if (!target.performCommand(Settings.SPAWNCOMMAND)) {
                                        target.teleport(RPG_Base.getIslandWorld().getSpawnLocation());
                                    }
                                } else {
                                    // Offline
                                    if (DEBUG)
                        ...
                        if (player.hasPermission("team.makeleader")) {
                            ...
                            if (targetPlayer.equals(playerUUID)) {
                                Util.sendMessage(player, ChatColor.RED + "Error at making player owner");
                                return true;
                            }
                            if (!plugin.getPlayers().inTeam(player.getUniqueId())) {
                            ...
                            if (plugin.getPlayers().inTeam(player.getUniqueId())) {
                                if (teamLeader.equals(player.getUniqueId())) {
                                    ...
                                    Util.sendMessage(player, ChatColor.RED + "Player is not in team");
                                } else {
                                    Util.sendMessage(player, ChatColor.RED + "Not your island!!!");
                                }
                            } else {
                                Util.sendMessage(player, ChatColor.RED + "Error at making player owner");
                            }
    ...
    private void warpPlayer(Player player, Location inFront, UUID foundWarp, BlockFace directionFacing, boolean pvp) {
        ...
        if (!event.isCancelled()) {
            ...
            if (warpOwner != null && !warpOwner.equals(player)) {
                Util.sendMessage(warpOwner, "Player teleported: " + player.getName());
            }
        }
    }
    ...
    private void chooseIsland(Player player) {
        // Get the NBTs that this player is eligible to use
        List<NBT> schems = getNBTs(player, false);
        //plugin.getLogger().info("DEBUG: size of NBTs for this player = " + schems.size());
        if (schems.isEmpty()) {
            // No NBTs - use default island
            newIsland(player);
        } else if (schems.size() == 1) {
            // Hobson's choice
            newIsland(player, schems.get(0));
    ...
    private void setResetWaitTime(final Player player) {
        resetWaitTime.put(player.getUniqueId(), Calendar.getInstance().getTimeInMillis() + Settings.resetWait * 1000L);
    }
